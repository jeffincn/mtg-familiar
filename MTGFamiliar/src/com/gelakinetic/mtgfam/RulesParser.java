...
public class RulesParser {
	...
	//Delimiting tokens
	//NOTE: If WotC changes their rules file format drastically, these may need to be changed as well
	...
	/**
	 * Returned from fetchAndLoad() if some of the rules/terms failed,
	 * but some succeeded.
	 */
	...
	public RulesParser(Date lastUpdated, CardDbAdapter mDbHelper, MainActivity main) {
	...
	/**
	 * Attempts to fetch the latest version of the rules and load it into the database.
	 * If the process is successful, true will be returned. Otherwise, false will be
	 * returned. This method should only be called if needsToUpdate() returns true.
		    ...
		    while(!line.equals(postRulesPreGlossary)) {
		    	...
		    	if(line.length() == 0) {
		    		...
		    		if(Character.isDigit(line.charAt(0))) {
		    			...
		    			if(currentRule != null) {
		    				currentRule.addExample(line.replace("{PW}", "{PWK}")
	                               .replace("{P/W}", "{PW}")
	                               .replace("{W/P}", "{WP}"));
		    			}
		    		}
		    ...
		    while(!line.equals(postGlossary)) {
		    	...
		    	if(line.length() == 0) {
		    		...
		    		if(currentTerm == null) {
		    			...
		    			currentTerm.addDefinitionLine(line.replace("{PW}", "{PWK}")
	                               .replace("{P/W}", "{PW}")
	                               .replace("{W/P}", "{WP}"));
		    		}
		    	}
	 ...
	 * @return SUCCESS if nothing goes wrong, ERRORS if some errors occur but some data
	 * is loaded, and FAILURE if everything fails and no data is loaded.
	 */
	public int loadRulesAndGlossary() {
		try {
			...
			for(RuleItem rule : rules) {
				try {
					mDbHelper.insertRule(rule.category, rule.subcategory,
							rule.entry, rule.text, rule.position);
				}
