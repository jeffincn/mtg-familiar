			...
			while (!c.isAfterLast()) {
				...
				if (verbose) {
					lCardlist.add(new TradeListHelpers().new CardData(card.name, tcgName, setCode, 0, 0, "loading", c.getString(c
							.getColumnIndex(CardDbAdapter.KEY_NUMBER)), c.getString(c.getColumnIndex(CardDbAdapter.KEY_TYPE)), c.getString(c
							.getColumnIndex(CardDbAdapter.KEY_MANACOST)), c.getString(c.getColumnIndex(CardDbAdapter.KEY_ABILITY)), c.getString(c
							.getColumnIndex(CardDbAdapter.KEY_POWER)), c.getString(c.getColumnIndex(CardDbAdapter.KEY_TOUGHNESS)), c.getInt(c
